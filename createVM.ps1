...
Import-Module "$PSScriptRoot\init" -Force

$init_Values = Do_Init
# Get the Si3 app configuration
$Si3Config = $init_Values[1]
$SQLVMConfig = $init_Values[2]
...
$vmConfig = New-AzureRmVMConfig -VMName 'myVM1' -VMSize Standard_A2 -AvailabilitySetId $as.Id | `
  Set-AzureRmVMOperatingSystem -Windows -ComputerName 'myVM1' -Credential $cred | `
  Set-AzureRmVMSourceImage -PublisherName MicrosoftWindowsServer -Offer WindowsServer `
  -Skus 2008-R2-SP1 -Version latest | Add-AzureRmVMNetworkInterface -Id $nicVM1.Id
...
# Create a virtual machine configuration
$vmConfig = New-AzureRmVMConfig -VMName 'myVM2' -VMSize Standard_A2 -AvailabilitySetId $as.Id | `
  Set-AzureRmVMOperatingSystem -Windows -ComputerName 'myVM2' -Credential $cred | `
  Set-AzureRmVMSourceImage -PublisherName MicrosoftWindowsServer -Offer WindowsServer `
  -Skus 2008-R2-SP1 -Version latest | Add-AzureRmVMNetworkInterface -Id $nicVM2.Id
...
# Create a virtual machine configuration
# $vmConfig = New-AzureRmVMConfig -VMName 'myVM3' -VMSize Standard_A2 -AvailabilitySetId $as.Id | `
#   Set-AzureRmVMOperatingSystem -Windows -ComputerName 'myVM3' -Credential $cred | `
#   Set-AzureRmVMSourceImage -PublisherName MicrosoftWindowsServer -Offer WindowsServer `
#   -Skus 2008-R2-SP1 -Version latest | Add-AzureRmVMNetworkInterface -Id $nicVM3.Id
